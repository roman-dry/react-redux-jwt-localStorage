...
function Register() {
  ...
  useEffect(() => {
    setErrMsg('');
  }, [name, lastName, email, pwd, matchPwd]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(REGISTER_URL, JSON.stringify(user1), {headers: { 'Content-Type': 'application/json' }});
      ...
      {success ? (
        ...
        <section>
          ...
          <p>
            Already registered?
            <br />
            <span className="line">{/* <Link to="/">Sign In</Link> */}</span>
          </p>
        </section>
      )}
    </>
